...
class Neural_Network():
    def __init__(self, sizes, epochs, activation="sigmoid", step_size=0.01, reg=0.0001, silent=False):
        ...
        # parameters
        self.params = {
            'W1': np.random.randn(input_layer+1, hidden_layer) / np.sqrt(hidden_layer),
            'W2': np.random.randn(hidden_layer+1, output_layer) / np.sqrt(output_layer)
        }
    ...
    def forward(self, x):
        ...
        memory = {}
        if len(x.shape) != 1:
            memory['A0'] = np.vstack([x.T, np.ones(x.shape[0])]).T

            memory['Z1'] = memory['A0'] @ params["W1"]
            memory['A1'] = np.vstack([activation(memory['Z1']).T, np.ones(memory['Z1'].shape[0])]).T
        else:
            memory['A0'] = np.hstack([x, 1])

            memory['Z1'] = memory['A0'] @ params["W1"]
            memory['A1'] = np.hstack([activation(memory['Z1']), 1])
    ...
    def backward(self, output, y_train):
        ...
        if len(output.shape) != 1:
            backward = np.dot(params['W2'], backward)[:, 0:-1] * activation_backward(memory['Z1'])
        else:
            backward = np.dot(params['W2'], backward)[0:-1] * activation_backward(memory['Z1'])
